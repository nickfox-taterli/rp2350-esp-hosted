...
#include <stdlib.h>
#include <time.h>
#include "slaveif/rpc_slave_if.h"
#include "string.h"
#include "adapter.h"
...
{
  ctrl_cmd_t *new_req = (ctrl_cmd_t*)pvPortCalloc(1, sizeof(ctrl_cmd_t));
  assert(new_req);
		...
		case RPC_ID__Event_ESPInit: {
			...
			if (strlen((char*)p_e->mac)) {
				ESP_LOGV(TAG, "%s ESP EVENT: SoftAP mode: connected station",
					get_timestamp_str(ts, MIN_TIMESTAMP_STR_SIZE));
				hosted_wifi_event_post(WIFI_EVENT_AP_STACONNECTED,
			...
			if (strlen((char*)p_e->mac)) {
				ESP_LOGV(TAG, "%s ESP EVENT: SoftAP mode: disconnected MAC",
					get_timestamp_str(ts, MIN_TIMESTAMP_STR_SIZE));
				hosted_wifi_event_post(WIFI_EVENT_AP_STADISCONNECTED,
			...
			hosted_wifi_event_post(WIFI_EVENT_STA_CONNECTED,
			...
			hosted_wifi_event_post(WIFI_EVENT_STA_DISCONNECTED,
			...
			hosted_wifi_event_post(wifi_event_id, 0, 0, HOSTED_BLOCK_MAX);
			...
			hosted_wifi_event_post(WIFI_EVENT_SCAN_DONE,
...
int rpc_wifi_get_mac(wifi_interface_t mode, uint8_t out_mac[6])
{
	...
	if (resp && resp->resp_event_status == SUCCESS) {
		...
		memcpy(out_mac, resp->u.wifi_mac.mac, BSSID_BYTES_SIZE);
...
int rpc_wifi_set_mac(wifi_interface_t mode, const uint8_t mac[6])
{
	...
	req->u.wifi_mac.mode = mode;
	memcpy(req->u.wifi_mac.mac, mac, BSSID_BYTES_SIZE);
...
//
//	req->app_free_buff_func = vPortFree;
...
int rpc_ota(const char* image_path)
{
	FILE* f = NULL;
...
int rpc_wifi_init(const wifi_init_config_t *arg)
{
	...
	memcpy(&req->u.wifi_init_config, (void*)arg, sizeof(wifi_init_config_t));
	resp = wifi_init(req);
...
int rpc_wifi_set_config(wifi_interface_t interface, wifi_config_t *conf)
{
	...
	memcpy(&req->u.wifi_config.u, conf, sizeof(wifi_config_t));
...
int rpc_wifi_get_config(wifi_interface_t interface, wifi_config_t *conf)
{
	...
	memcpy(conf, &resp->u.wifi_config.u, sizeof(wifi_config_t));
...
int rpc_wifi_scan_start(const wifi_scan_config_t *config, bool block)
{
	...
	if (config) {
		memcpy(&req->u.wifi_scan_config.cfg, config, sizeof(wifi_scan_config_t));
...
int rpc_wifi_scan_get_ap_records(uint16_t *number, wifi_ap_record_t *ap_records)
{
	...
	if (resp && resp->resp_event_status == SUCCESS) {
		...
		memcpy(ap_records, resp->u.wifi_scan_ap_list.out_list,
...
int rpc_wifi_sta_get_ap_info(wifi_ap_record_t *ap_info)
{
	...
	if (resp && resp->resp_event_status == SUCCESS) {
		memcpy(ap_info, resp->u.wifi_scan_ap_list.out_list,
