...
static QueueHandle_t rpc_rx_q = NULL;
static QueueHandle_t rpc_tx_q = NULL;

static TaskHandle_t rpc_rx_thread_hdl;
static TaskHandle_t rpc_tx_thread_hdl;
...
static void rpc_async_timeout_handler(TimerHandle_t xTimer);
...
{
	/* 使用FreeRTOS的信号量机制来通知任务 */
	xSemaphoreGive(rpc_tx_sem);
}
...
static int process_rpc_tx_msg(ctrl_cmd_t *app_req)
{
	 ...
	 * For sync procedures, xSemaphoreTake takes care to
	...
	if (app_req->rpc_rsp_cb) {
		...
		async_timer_hdl = xTimerCreate("RpcTimer",
							 pdMS_TO_TICKS(app_req->rsp_timeout_sec * 1000),
							 pdFALSE,
							 (void*)app_req,
							 rpc_async_timeout_handler);
		if (!async_timer_hdl) {
...
fail_req:
	...
	if (app_req->rpc_rsp_cb) {
		...
		if (!xQueueSend(rpc_rx_q, &elem, pdMS_TO_TICKS(HOSTED_BLOCK_MAX)) != pdPASS) {
		// if (xQueueSend(rpc_rx_q, &elem, HOSTED_BLOCK_MAX)) {
...
static int process_rpc_rx_msg(Rpc * proto_msg, rpc_rx_ind_t rpc_rx_func)
{
	...
	/* 3. Check if it is response msg */
		...
		if (async_timer_hdl) {
			...
			xTimerDelete(async_timer_hdl,portMAX_DELAY);
...
static void rpc_tx_thread(void  *arg)
{
	...
	while (1) {
		...
		xSemaphoreTake(rpc_tx_sem, portMAX_DELAY);

		if (!xQueueReceive(rpc_tx_q, &app_req, HOSTED_BLOCK_MAX)) {
...
{
	/* 使用FreeRTOS创建RPC接收线程 */
	xTaskCreate(rpc_rx_thread,          // 线程函数
				"rpc_rx",               // 线程名称
				RPC_TASK_STACK_SIZE,    // 栈大小
				NULL,                   // 参数
				RPC_TASK_PRIO,          // 优先级
				&rpc_rx_thread_hdl);    // 任务句柄

	/* 使用FreeRTOS创建RPC发送线程 */
	xTaskCreate(rpc_tx_thread,          // 线程函数
				"rpc_tx",               // 线程名称
				RPC_TASK_STACK_SIZE,    // 栈大小
				NULL,                   // 参数
				RPC_TASK_PRIO,          // 优先级
				&rpc_tx_thread_hdl);    // 任务句柄

	/* 检查线程是否创建成功 */
	if (rpc_rx_thread_hdl == NULL || rpc_tx_thread_hdl == NULL) {
		ESP_LOGE(TAG, "Thread creation failed for rpc_tx/rx_thread");
		return FAILURE;
	}
...
{
	int ret1 = pdFAIL, ret2 = pdFAIL;

	/* 删除RPC接收线程 */
	if (rpc_rx_thread_hdl != NULL) {
		vTaskDelete(rpc_rx_thread_hdl);
		ret1 = pdPASS;
	}

	/* 删除RPC发送线程 */
	if (rpc_tx_thread_hdl != NULL) {
		vTaskDelete(rpc_tx_thread_hdl);
		ret2 = pdPASS;
	}
...
static ctrl_cmd_t * get_response(int *read_len, ctrl_cmd_t *app_req)
{
	...
	if (!xQueueReceive(rpc_rx_q, &elem, HOSTED_BLOCK_MAX)) {
...
static int post_sync_resp_sem(ctrl_cmd_t *app_resp)
{
	...
	for (i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
		if (sync_rsp_table[i].uid == app_resp->uid) {
			return xSemaphoreGive(sync_rsp_table[i].sem);
		}
	}
...
static int set_sync_resp_sem(ctrl_cmd_t *app_req)
{
	...
	if (app_req->rx_sem)
		vSemaphoreDelete(app_req->rx_sem);
	...
	if (exp_resp_msg_id >= RPC_ID__Resp_Max) {
		...
		/* For sync, set sem */
		app_req->rx_sem = xSemaphoreCreateBinary();
		xSemaphoreTake(app_req->rx_sem, 0);
...
static int wait_for_sync_response(ctrl_cmd_t *app_req)
{
	...
	for (i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
		if (sync_rsp_table[i].uid == app_req->uid) {
			ret = xSemaphoreTake(sync_rsp_table[i].sem, timeout_sec);
			vSemaphoreDelete(sync_rsp_table[i].sem);
			// clear table entry
...
static void rpc_async_timeout_handler(TimerHandle_t xTimer)
{
	 ...
	 * */
	ctrl_cmd_t *app_req = (ctrl_cmd_t *)pvTimerGetTimerID(xTimer);;
...
int rpc_send_req(ctrl_cmd_t *app_req)
{
	...
	if (!xQueueSend(rpc_tx_q, &app_req, HOSTED_BLOCK_MAX)) {
...
int rpc_core_deinit(void)
{
	...
	if (rpc_rx_q) {
		vSemaphoreDelete(rpc_rx_q);
	}

	if (rpc_tx_q) {
		vSemaphoreDelete(rpc_tx_q);
	}

	vSemaphoreDelete(rpc_tx_sem);
	configASSERT(rpc_tx_sem == NULL);

	if (async_timer_hdl) {
		/* async_timer_hdl will be cleaned in g_h.funcs->_h_timer_stop */
		xTimerDelete(async_timer_hdl, 0);
		async_timer_hdl = NULL;
	}
...
int rpc_core_init(void)
{
	...
	/* semaphore init */
	rpc_tx_sem = xSemaphoreCreateCounting(H_MAX_SYNC_RPC_REQUESTS +
			H_MAX_ASYNC_RPC_REQUESTS,0);
	if (!rpc_tx_sem) {
	...
	rpc_rx_q = xQueueCreate(RPC_RX_QUEUE_SIZE,
			sizeof(esp_queue_elem_t));

	rpc_tx_q = xQueueCreate(RPC_TX_QUEUE_SIZE,
			sizeof(void *));
