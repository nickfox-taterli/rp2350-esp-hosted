...
struct serial_drv_handle_t* serial_drv_open(const char *transport)
{
	...
	serial_drv_handle = (struct serial_drv_handle_t*) pvPortCalloc
...
{
	/* Create semaphore using FreeRTOS API */
	readSemaphore = xSemaphoreCreateCounting(
...
static void rpc_rx_indication(void)
{
	...
	if(readSemaphore) {
		/* 使用FreeRTOS的信号量通知机制 */
		xSemaphoreGive(readSemaphore);  // 释放信号量，通知等待的任务
	}
}
