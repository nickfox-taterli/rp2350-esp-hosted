...
__attribute__((weak)) esp_err_t esp_wifi_remote_channel_rx(void *h, void *buffer, void *buff_to_free, size_t len)
{
...
__attribute__((weak)) esp_err_t esp_wifi_remote_channel_set(wifi_interface_t ifx, void *h, esp_remote_channel_tx_fn_t tx_cb)
{
	if (ifx == WIFI_IF_STA) {
		s_channel[0] = h;
		s_tx_cb[0] = tx_cb;
		...
		esp_remote_channel_tx_fn_t *tx, const esp_remote_channel_rx_fn_t rx)
{
	...
	eh_chan = pvPortCalloc(sizeof(struct esp_remote_channel), 1);
	assert(eh_chan);
